...
- **`storyteller`** (`.claude/output-styles/storyteller.md`) — 게임마스터 인격(story 모드, 2인칭).
- **`companion`** (`.claude/output-styles/companion.md`) — 1:1 캐릭터챗(chat 모드, 캐릭터가 페르소나에게
  1인칭 응답). 씬의 `mode`로 settings에서 선택, 또는 `/config` → **Output style** 수동 설정.
  [guide/output-style](../guide/output-style.ko.md) 참고.
...
| `new-character` | `/new-character` | OKF 캐릭터(인사말+예시대화 포함)를 스토리에 추가. |
| `new-scene` | `/new-scene` | 기존 캐릭터로 현재 스토리에 새 상황 시작. |
